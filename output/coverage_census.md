...
- 生成: 2026-08-19T10:37:29+00:00（`python3 scripts/coverage_census.py`）
- 数え方の正本: tasks/xprice_reform_review.md §8（P-08a 裁定 2026-08-17）。
  **入力件数ではなく「独立ドライバー×稼働取得経路×関門通過カード」の重複除外集合**を数える。

## いまの実効カバレッジ

- **実効ドライバー数 = 36**（3倍の目標値 = **108**）
- 銘柄を出せる会社数 = 43 社（関門通過カードのみ・重複除外）
- 参考: 全ドライバー 89 / 稼働中 61（入力 126 件から重複 37 件を統合した後）
...
- 🔓 **止まっているレーンが開いた場合の実効**: 40 ドライバー / 49 社（増える分: memory-asp-estat, x:cpu-logic, x:secondary-resale, x:shokuhin-neage）

## レーン別の稼働

| レーン | 登録数 | 稼働 |
|---|---|---|
| B2B価格 | 64 系列 | 直近取得OK 64 系列 |
| news_shock | 20 クエリ | 停止（最終 run=2026-08-19T10:06 だが成功クエリ 0） |
