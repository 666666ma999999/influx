...
- 生成: 2026-08-19T05:03:42+00:00（`python3 scripts/coverage_census.py`）
- 数え方の正本: tasks/xprice_reform_review.md §8（P-08a 裁定 2026-08-17）。
  **入力件数ではなく「独立ドライバー×稼働取得経路×関門通過カード」の重複除外集合**を数える。

## いまの実効カバレッジ

- **実効ドライバー数 = 34**（3倍の目標値 = **102**）
- 銘柄を出せる会社数 = 41 社（関門通過カードのみ・重複除外）
- 参考: 全ドライバー 87 / 稼働中 59（入力 124 件から重複 37 件を統合した後）
...
- 🔓 **止まっているレーンが開いた場合の実効**: 38 ドライバー / 47 社（増える分: memory-asp-estat, x:cpu-logic, x:secondary-resale, x:shokuhin-neage）

## レーン別の稼働

| レーン | 登録数 | 稼働 |
|---|---|---|
| B2B価格 | 62 系列 | 直近取得OK 62 系列 |
