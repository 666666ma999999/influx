...
PATTERNS = {
    ...
    "立会外分売": r"立会外分売",
    "希望退職": r"希望退職",
    # 名人収穫(§8-5)の「見込みあり」家系をTDnet表題で機械化したもの（2026-07-27 追加）
    "完全子会社化(親子上場解消)": r"完全子会社化|株式交換.*完全子会社|簡易株式交換",
    "TOB開始": r"公開買付.*開始|公開買付届出|公開買付けの開始",
    "TOB意見表明": r"意見表明",
    ...
    # 自社株買い決議（catalog §2 #12・2026-08-02 追加）: 「新規の取得決定」だけを拾う。
    # 進捗/完了(状況・結果・終了)・希薄化側(処分)・別イベント(消却・端数買取り・株式報酬)は
    # 型分布の実測（18年86,580件→型4,398種）に基づき負の先読みで除外。
    "自社株買い決議(#12)": (
        r"\A(?!.*(?:状況|結果|終了|処分|消却|買取り|報酬))"
        r".*(?:自己株式の?取得に係る事項の決定|自己株式の市場買付|立会外買付取引による自己株式の買付け)"
    ),
}
...
def main() -> int:
    ...
    for name in sorted(by_ev, key=lambda k: -len(by_ev[k])):
        ...
        if s and s[0] >= a.min_n:
            ...
            lines.append(f"| {name} | {n:,} | {ev*100:+.2f}% | {hit*100:.1f}% | {med*100:+.2f}% |")
    lines += ["", "## 読み方（重要）",
              # 2026-08-15 訂正: 旧「9.5%」は tasks/stock_algo_kpi_loop.md:669 の**2026-02〜04の過熱3ヶ月
              # ローカル値**（3月13.0%/4月14.8%）を全期間の市場基準として誤用していた。同定義（終値+20%/20bd・
              # hit20 = gross>=0.20）の全期間確定値は catalog §6（2026-07-06・118ヶ月・n=58,497）の **4.1%**。
              # 誤用の実害: 全イベントを2.3倍厳しい基準で割っており、17イベント中16本が「ベース未満」と判定
              # されていた（正しい基準では上位6本が lift 1.5倍超）。高値タッチ版と比べる場合のみ 8.5% を使う。
              "- **+20%到達率を市場ベースレート（catalog §6 確定値 4.1%・終値+20%/20bd・全期間118ヶ月）と"
              "比べる**のが第一。下回るなら候補にならない。※高値タッチ判定と比べる時だけ 8.5% を使う"
              "（本表の hit20 は終値ベースなので 4.1% が正）。",
