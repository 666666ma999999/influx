...
DEFAULT_QUERIES = [
    ...
    # gen8 2026-08-12（**在庫ゼロ5グループの空白埋め**・オーナー指示「在庫ゼロ5つにクエリを作って」）:
    # gen7 は「採用率が高いのにクエリが無い」3グループを埋めた。今回はその逆側＝
    # **在庫が1件も無い5グループ**（G4/G5/G8/G10/G11）。34→39 の純増5。
    # 判定の既定値は引き継ぎ書 tasks/handoff-2026-08-11.md §2 判断待ち#2「作って試す
    # （各1本・4週で0件なら降ろす）」。**降ろす期限 2026-09-09**（4週後の intent_coverage で判定）。
    # 選定手順は gen7 と同じ: コーパス1,807件（grok-twittora 8本＋x-tracer 16本＋bookmarks＋攻め棚）で
    # 実在を数え、≤3語・R3非衝突・「固有名詞×作業の言い回し」に寄せた。
    # ⚠️ G5/G10/G11 は台帳の代表語がコーパスに**ほぼ実在しない**（0〜1件）。0件は不在の証拠に
    #    ならない（既存クエリが釣った結果のコーパスだから）ので、周辺の実在語に置き換えて打つ。
    #    実在の弱い順 = G10 > G11 > G8 > G5 > G4 ＝ この順で退役候補。
    # ⚠️ **計器側と対で変える**: make_article `config/intent_groups.yaml` の match_terms に
    #    同じ語を入れないと、集まっても在庫に計上されず ❌ が永久に消えない（C3 の片側ドリフト）。
    #    今回 G5「判断基準」・G8「リファクタ」を同一変更で追加済み。
    "Claude 寝ている間",                # 意図G4 無人運転（在庫0・既存の "Claude Skills Routines" は
                                       #   3語ANDで狭すぎて0件だった＝語を替える）
                                       #   実在: 「寝ている間」6件/著者6人（gen8で最強・台帳の代表語そのもの）
    "Claude 判断基準",                  # 意図G5 暗黙判断の言語化（在庫0）
                                       #   ⚠️ 台帳の代表語「意思決定の記録」「暗黙知」は実在**0件**。
                                       #   周辺で唯一実在した「判断基準」6件/著者5人に置換して打つ
    "Claude Code リファクタ",           # 意図G8 大掃除リファクタ（在庫0）
                                       #   実在: 「リファクタ」2件/著者2人・「負債」3件/著者3人。
                                       #   台帳の「大掃除」は1件/1人（単一著者=riku720720）で弱い
    "Claude Learning Mode",            # 意図G10 AIで学び直す（在庫0）
                                       #   ☆gen8 最弱: 実在1件/著者1人（MakeAI_CEO）。
                                       #   最多だった「独学」7件/6人は**教材紹介＝R3寄り**なので採らない
                                       #   （2026-08-11 に generative-ai-for-beginners を同じ理由で落とした）
    "リバースプロンプティング",          # 意図G11 質問のさせ方（在庫0）
                                       #   実在1件/著者1人だが**造語でノイズが最小**。
                                       #   「質問して」3件/3人・「壁打ち」2件/2人は一般語すぎて他意図を拾う
    # gen8b 2026-08-12（**最後の ❌ を潰す**・オーナー指示「G6にも1本足して」）:
    # G6 は在庫2件・採用1件（採用率50%）と実績があるのにクエリが1本も無かった＝
    # gen7 が埋めた G7/G9/G1 と同じ型（実績があるのに入口が無い）。これで ❌ は 0 になる。
    "AI同士 レビュー",                  # 意図G6 AI同士を議論させる（在庫2→採用1）
                                       #   実在 2件/著者2人。**目視で2件とも G6 の芯**（相互レビュー／不一致の資産化）＝精度100%。
                                       #   ⚠️ 却下した候補と理由（いずれも実文を目視して決めた）:
                                       #     ・"Claude Codex レビュー" 8件/8人と最多だが、目視すると **G6 の芯は3件だけ**（3/8）。
                                       #       残り5件は tips集・公式ガイド解説・ニュースまとめ・OSS紹介＝棚の89%を占める
                                       #       「方向性の外側」をさらに太らせる。件数で選ぶと精度を落とす実例。
                                       #     ・"AI同士" 単体 5件/5人 → 5件中2件が**新機能速報(G12)**（セッション間メッセージ）に流出。
                                       #       "レビュー" を足すとこの2件が落ちて芯だけ残る。
                                       #     ・台帳の代表語「敵対レビュー」は実在0件・「敵対的レビュー」1件/1人で打てない。
                                       #   ⚠️ 計器の紐付け条件: **クエリ文字列自体が match_terms を含む**こと
                                       #     （`intent_group_coverage.build()` は `hits(query, terms)` で判定）。
                                       #     本クエリは "ai同士" を含むので自動で G6 に紐づく。含まない語を選ぶと
                                       #     クエリを足しても ❌ のままになる（2026-08-12 に実際に踏んだ）。
    # gen8c 2026-08-12（G13 新設に伴う入口づくり・オーナー裁定P3+「今すぐ投入」）:
    # G13 SNS/X運用の自動化（オーナー事業ドメイン）は 2026-08-12 新設。在庫3・採用1 だが収集クエリ0本＝
    # 新規流入がブックマーク頼みだった。発端記事（MakeAI_CEO『Claude Code×Grok 活用術5選』imp2.5万）と同型を拾う。
    "SNS運用 Claude",                   # 意図G13 SNS/X運用の自動化（在庫3→採用1）
                                       #   ⚠️ 却下した候補と理由（実文を目視して決めた・2026-08-12）:
                                       #     ・"SNS運用" 単体 → x-tracer 8月実在2件の**目視2件とも芯ゼロ**（AI社員ネタ・煽り）＝
