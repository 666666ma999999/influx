...
def run_update(records: list[dict]) -> tuple[list[dict], dict, dict]:
    """pending_entry埋め + open進行判定を1パス実行する共通処理（daily_screen.pyからも呼ばれる）。"""
    ...
    return records, fill_diag, open_diag
...
def main() -> int:
    ...
    if not args.scoreboard_only and records:
        records, fill_diag, open_diag = run_update(records)
