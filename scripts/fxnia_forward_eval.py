...
# ⚠️ 2026-08-16 誤前提の申告（**値は変更していない・変更にはユーザー裁定が必要**）:
# 0.095 は「市場ベースレート」ではなく tasks/stock_algo_kpi_loop.md:669 の 2026-02〜04（過熱3ヶ月・
# 3月13.0%/4月14.8%）のローカル値。同定義（終値+20%/20bd）の全期間確定値は docs/stock-algo-kpi-catalog.md
# §6 の 4.1%（118ヶ月・n=58,497）。つまり本スクリプトの合格条件c3は市場平均の2.3倍を要求している。
# **勝手に 0.041 へ緩めない**: これは事前登録された凍結条件であり、データを見た後に基準を下げるのは
# 後出しチューニング（prereg-freeze-cycle 違反）。訂正するなら「誤った前提に基づく凍結条件の改定」として
# ユーザー承認＋改定記録を経ること。現状は誤前提のまま厳しい側に倒れている＝偽陽性は出さない。
BASELINE = 0.095   # ⚠️上記参照: 市場ベースレートではなく過熱3ヶ月ローカル値（凍結条件のため未変更）
STOP_CALLS = 60
STOP_MONTHS = 4
