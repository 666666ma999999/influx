...
>
> 🕰 **鮮度の警告**: 下の「モジュール構成」「データフロー」節は **2026-05-02 が最終更新**で、
> 7〜8月に新設された X収集の主力スクリプト（`x_search_collect_twittora.py` / `search_x_articles.py` /
> `fetch_x_article.py` 等）を**載せていない**。**現況は上の2枚を見ること**。本ファイルで信頼できるのは
> §環境変数 以降の参照データ（設定値・分類定義）。
