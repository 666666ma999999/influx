# influx 参照データ（設定値・スキーマ・分類定義）

> ⚠️ **役割の分離（2026-08-09・グローバル規約 rules/42 改訂に伴う整理）**
> **機能マップ（いま何が動いているか・工程・道具・I/O）はこのファイルではありません**:
> - **株アルゴ研究** → `influx-stock-algo-architecture.md`（repo 直下）
> - **X収集基盤** → `influx-architecture.md`（repo 直下）
>
> **本ファイルの役割 = 詳細リファレンス**（環境変数一覧・collect_tweets のオプション・インフルエンサーグループ定義・
> 分類カテゴリ・LLM分類の特別ルール・カテゴリ→テンプレート対応表）。上の2枚から「詳細は本ファイル」とリンクされる。
>
> 🪦 **退役進行中（2026-08-29 実測で節ごとに仕分け済み・断捨離監査 I-19）**。読んでよい節と読んではいけない節:
> - ❌ **モジュール構成・データフロー**: 2026-05-02 が最終更新で、7〜8月の X収集主力スクリプトを載せていない。
>   さらに `collect_tweets.py` / `classify_tweets.py` / `viewer.html` は **2026-08-29 に退役済み**（実体が無い）
> - ❌ **インフルエンサーグループ定義**: 文書は6グループだが実体は **8グループ**（`collector/config.py` 実測）
> - ❌ **collect_tweets オプション**: 退役済み。`--scrolls` の既定も文書10 vs 実装20 で不一致だった
> - ❌ **「Few-shot 例は46例」**: 実体は **51例**（`data/few_shot_examples.json`）
> - 🚚 **環境変数**: `influx-architecture.md` §5.2 へ移送済み（こちらは読まない）
> - ⭕ **分類カテゴリ・カテゴリ→テンプレート対応表・データスキーマ**: `collector/config.py` と一致を実測（2026-08-29）。
>   ただし**正本はコード側**＝迷ったら `collector/config.py` を読む
>
> **現況の機能は `influx-architecture.md` / `influx-stock-algo-architecture.md` が正本**。
