...
def _map(tier: str, stype: str = "price") -> dict:
    return {
        "version": 1,
...
def _errors(tier: str, watch: str, note: str, stype: str = "price") -> list[str]:
    with tempfile.TemporaryDirectory() as d:
        cp = Path(d) / "center_pin.jsonl"
        cp.write_text(json.dumps({"code": "9999", "name": "テスト社", "watch": watch,
                                  "note": note}, ensure_ascii=False) + "\n",
                      encoding="utf-8")
        return x.validate(_map(tier, stype), center_pin=x.load_center_pin(cp), query_ids={"q1"})
