...
> ⚠️ P9 フォロワー計測（`fetch_followers.py`）は配線先が未確認（`last_verified` 時点）。
...
| launchd | X収集の定時7本。plist は薄く、リトライ・通知は runner 側 | ログは `~/.claude/state/<job>.{out,err}.log` に集約 | cron／GitHub Actions | personal |
...
|---|---|---|---|---|
| `refresh-x-cookies` | skill(自前) | macOS Chrome のプロファイルから X の Cookie を抜いて暗号化保存 | **VNC Playwright 経路は bot 検知強化で廃止（2026-04-21）**。Chrome に X ログイン済みなら1コマンド | 公式 API（費用の裁定で不採用） |
| `x-post-to-kpi` | skill(自前) | X の投稿URLを KPI 仮説として取り込むかを判定する手順 | **投稿を読み切ってから判定**（本文・長文フィールド・添付図・スレッド続き）。既存カタログと重複照合し、実装可否まで見てから事前登録へ | — |
...
- **未確認**: P9 フォロワー計測の配線先／週次ピック runner は 7/26 以降不発だったが 2026-08-08 にループ型で復旧（別途）
