# bookmarks_keyword_extraction_v2 — X検索キーワード群の世代提案プロンプト

バージョン: v2（2026-07-12 新設。v1からの差分は「評価マークの反映」節への note 解釈の追記のみ — Webページ評価UI追加に伴い、pending_evaluations の各項目に任意の `note`（🔁/🔍選択時の固定チップ）が付くようになったため。**このファイルは直接書き換えず、変更時は `_v3.md` を新設**する — influx プロンプトバージョニング規約）
...
1. `output/bookmarks/keyword_worklist.json` — 必読。フィールド:
   ...
   - `pending_evaluations`: 人間が付けた評価マーク（✅使えた/🔁多すぎ/🔍少なすぎ/❌使わない）+ 任意の `note`。`note` は 🔁/🔍 時のみ付きうる固定チップ（`min_favesを上げる`/`min_favesを下げる`/`絞り込み語を足す`/`語を減らす`）で、指定があれば下記「評価マークの反映」の note 解釈規則に従う。空文字ならチップ未選択（従来通りの一般的な絞る/広げる判断でよい）
...
3. 🔁多すぎ → 絞る方向に改訂（語を足す・min_faves を上げる・期間を絞る）。**`note` が `min_favesを上げる` の場合、キーワード集合・クエリ構造は変えず `min_faves:` の数値だけを引き上げる。`note` が `絞り込み語を足す` の場合、キーワードは変えず絞り込み語（追加の OR 語・`-`除外・`lang:`/`filter:` 等）を足す方向で改訂する。`note` が空なら従来通りどちらの手段を使ってもよい**
4. 🔍少なすぎ → 広げる方向に改訂（語を減らす・OR を使う・min_faves を下げる）。**`note` が `min_favesを下げる` の場合、キーワード集合・クエリ構造は変えず `min_faves:` の数値だけを引き下げる。`note` が `語を減らす` の場合、キーワード/絞り込み語側（除外語・限定的な語）を減らす方向で改訂する。`note` が空なら従来通りどちらの手段を使ってもよい**
