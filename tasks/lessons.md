...
## 2026-08-16: 全期間 in-sample の「合格」は期間を割ると消えることがある — 多重比較調整ではこの型を捕まえられない
- 症状: TDnet再評価で株式分割が枠S通過（全期間 lift 2.73[CI下限1.78]・EV(stop8)+3.41%）。**Bonferroni 調整（17イベント同時・α=0.05/17）後も CI下限 1.38〜1.46 で 1.2 を維持**＝統計的な多重比較の観点では堅牢に見えた。しかし事前固定の期間分割（前半201609〜202112 / 後半202201〜202606）で割ると、**後半は lift 1.82[CI下限0.78]・EV(なし)片側下限 −0.87% で符号反転**＝NO-GO。配当予想の修正（枠F通過）も後半で lift 0.90・EV下限 −0.25% と根拠2条件が両方崩壊
- 実害の可能性: この確認を挟まずに事前登録へ進んでいたら、**前半（〜2021）の遺物を前向き検証に載せて数年を溶かす**ところだった
- 切り分けの決め手: 月別ベースレートは前半3.79%→後半4.44%と**上昇**しているのに、株式分割の到達率は14.8%→7.5%へ**半減**。市場全体の変化では説明できず、イベント固有の減衰と読める。発火頻度は年25.3→41.6件と増加（＝珍しくなくなり材料視されなくなった可能性。機序は未検証）
- 再発防止の型: **in-sample で入場ゲートを通った候補は、事前登録の前に「事前固定の1分割で前半/後半」を必ず測る**。分割点は測定前に1通りだけ決めて凍結する（複数試して良い方を選ぶのは後出し）。判定規約（GO/条件付きGO/NO-GO）も測定前に固定し、結果を見てから語を変えない
- 関連: 多重比較（横方向の偽陽性）と時期偏在（縦方向の陳腐化）は**別の失敗モード**であり、前者の調整では後者を検出できない
