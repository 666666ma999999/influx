...
### 2026-07-27 ユーザー裁定: 値上がり検出は「商品市況起点レーン」単独運用で確定

- **裁定**: 現状は商品等の値上がり（実物価格型）だけでよい。開示監視・テーマ・需給などの
  別レーンは**追加しない**（「色々と混ぜると混乱するので」）。
- 含意: カバー範囲は急騰全体の約2〜3割（実物価格型）に限定されることを了解済み。
  **AI側から別レーン新設を再提案しない**（ユーザーが明示的に求めた時のみ）。
- 稼働構成（この裁定時点）: ①X値上がり監視 price_watch（毎日22:10・アラートは8/2頃〜）
  ②eBay ask 週次（pokeca-invest・手動1行）③業界卸値は必要時に手動確認。
  アラート発火 → 銘柄マッピング＋5チェック検証 → 判断材料の表、が運用フロー。

### 2026-07-27 追補5: 石油2銘柄の精密検査 — 両方「見送り」・スキャンの期間ズレ錯覚を2例目確認

> 出所: 独立レビュアー2本（1662/1605・一次資料実読: INPEX/JAPEX決算説明資料・株探材料記事・IFISコンセンサス）。
> 買わない判定で両者独立収束のため異モデル最終判定は省略（前提=買い推奨前の確認、が消滅）。

- **INPEX(1605): 見送り**。同一13週で Brent -27% vs 株 -7.7% ＝株が原油を19pt アウトパフォーム（未織り込みの逆）。
  感応度は公式 $1=55億円だが期中逓減で現時点 $1=19億円。原油+20%相当は 5/13上方修正（3,300→3,500-4,500億）で
  織り込み済み。6月に2社が目標4,600-4,700円へ引上げ済み。PER 9.6-12.4倍で歪みなし。
- **JAPEX(1662): 見送り**。13週で WTI -16% vs 株 -21.9%（β1.36 の追随＋5/13決算週-8.8%＋減配65→45円）。
  さらに**会社自身が「今回の中東起因の原油高は差引△72億円の減益」と開示**（LNGスポット代替調達△156億・
  ガラフ油田通年停止△39億が恩恵を打消し）。アナリスト4人全員中立。唯一の逆張り筋は「緊張緩和で買い」
  （ガラフ再開+39億・仮説と真逆・小規模）。
- **確定知見（2例目）**: rising-goods スキャンの「商品1ヶ月% vs 株13週%」比較は**期間ズレの錯覚を系統的に生む**
  （SMM に続き油2銘柄で再現・4週同士で測ると株は反応済み）。今後のスキャン表は**同一窓の商品変化率列を必須**とする
  （5チェック関門①の掲載式への昇格）。
- 現時点の総括: 2026-07-27 時点で「商品高×未織り込み×エッジあり」の生存候補は**ゼロ**（8銘柄検査・8棄却）。
  次の候補源は新規の値上がり初動（price_watch アラート・卸値変曲・eBay ask週次急変）のみ。
